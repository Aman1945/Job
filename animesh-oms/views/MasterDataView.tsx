...
import { 
  ...
  Search, FileUp, AlertCircle, FileCheck, Layers, Camera, Image as ImageIcon
} from 'lucide-react';
...
const MasterDataView: React.FC<MasterDataViewProps> = ({ 
  ...
  const processSmartMerge = (tab: NexusTab, rows: any[]) => {
    ...
    if (tab === 'User Master' || tab === 'Delivery Person') {
      // Added explicit Map typing to prevent 'unknown' inference on retrieval which fixes property access errors
      const currentMap = new Map<string, User>((users || []).map(u => [u.id.toLowerCase(), u]));
      ...
      // Added explicit Map typing to prevent 'unknown' inference on retrieval which fixes property access errors (e.g., line 143 imageUrl)
      const currentMap = new Map<string, Product>((products || []).map(p => [p.skuCode.toLowerCase(), p]));
      rows.forEach((row: any) => {
        ...
        const productData: Product = {
          ...
          baseRate: mrp,
          stock: 0,
          // Fixed 'Property imageUrl does not exist on type unknown' by explicitly typing the Map
          imageUrl: row.imageUrl || currentMap.get(sku.toLowerCase())?.imageUrl
        };
      ...
      // Added explicit Map typing to prevent 'unknown' inference on retrieval which fixes property access errors
      const currentMap = new Map<string, Customer>((customers || []).map(c => [c.id.toLowerCase(), c]));
      rows.forEach((row: any) => {
      ...
      // Added explicit Map typing to prevent 'unknown' inference on retrieval which fixes property access errors
      const currentMap = new Map<string, ODMaster>((odMaster || []).map(o => [o.customerId.toLowerCase(), o]));
      rows.forEach((row: any) => {
                ...
                <table className="w-full text-left whitespace-nowrap">
                   <thead className="bg-white text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b sticky top-0 z-20">
                      <tr>
                        ...
                        {activeTab === 'Material Master' && <> <th className="px-8 py-5">Visual</th> <th className="px-6 py-5">Code</th> <th className="px-6 py-5">Product Name</th> <th className="px-6 py-5 text-right">MRP</th> <th className="px-6 py-5 text-right">GST%</th> <th className="px-6 py-5">Country</th> </>}
                        ...
                        <tr key={item.id || item.productCode || item.customerId || item.skuCode || Math.random()} className="hover:bg-white group transition-colors">
                          ...
                          {activeTab === 'Material Master' && <> 
                            <td className="px-8 py-4">
                              <div className="w-12 h-12 bg-slate-100 rounded-xl border border-slate-200 overflow-hidden flex items-center justify-center shadow-inner">
                                {item.imageUrl ? <img src={item.imageUrl} className="w-full h-full object-cover" /> : <ImageIcon className="text-slate-300" size={16} />}
                              </div>
                            </td>
                            <td className="px-6 py-4 font-mono text-indigo-600">{item.skuCode}</td> 
                            <td className="px-6 py-4">{item.name}</td> 
                            <td className="px-6 py-4 text-right font-black">₹{item.price?.toLocaleString() || 0}</td> 
                            <td className="px-6 py-4 text-right">{item.gst || 0}%</td> 
                            <td className="px-6 py-4 text-slate-400">{item.countryOfOrigin || 'N/A'}</td> 
                          </>}
                    ...
                    <div className="space-y-10">
                      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                        <div className="lg:col-span-1">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest px-1 block mb-3">Product Visual</label>
                          <div 
                            onClick={() => productImgInputRef.current?.click()}
                            className={`group relative aspect-square rounded-[32px] border-4 border-dashed transition-all flex flex-col items-center justify-center gap-4 cursor-pointer overflow-hidden ${formData.imageUrl ? 'border-indigo-600 bg-white' : 'border-slate-200 bg-slate-50 hover:bg-indigo-50 hover:border-indigo-300'}`}
                          >
                            {formData.imageUrl ? (
                              <>
                                <img src={formData.imageUrl} className="absolute inset-0 w-full h-full object-cover" alt="Product Preview" />
                                <div className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                  ...
                                  <p className="text-[10px] font-black text-white uppercase tracking-widest">Update Snapshot</p>
                                </div>
                              </>
                            ) : (
                              <>
                                <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center text-slate-300 group-hover:text-indigo-500 group-hover:scale-110 transition-all shadow-sm">
                                  <Camera size={32} />
                                </div>
                                <div className="text-center">
                                  <p className="text-xs font-black text-slate-700 uppercase tracking-widest px-4 leading-tight">Upload Product Image</p>
                                </div>
                              </>
                            )}
                          </div>
