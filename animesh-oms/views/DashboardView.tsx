...
const DashboardView: React.FC<DashboardViewProps> = ({ orders, onViewOrder }) => {
  const departmentHealth = [
    ...
    { label: 'Logistics', status: 'Transit Hub', count: orders.filter(o => o.status === OrderStatus.OUT_FOR_DELIVERY || o.status === OrderStatus.PICKED_UP || o.status === OrderStatus.DISPATCHED).length, icon: <Truck size={14}/>, color: 'text-indigo-600', bg: 'bg-indigo-50' },
  ];

  const pieData = [
    { name: 'Pending', value: orders.filter(o => o.status.includes('Pending')).length, color: '#f59e0b' },
    { name: 'In Transit', value: orders.filter(o => o.status === OrderStatus.OUT_FOR_DELIVERY || o.status === OrderStatus.PICKED_UP || o.status === OrderStatus.DISPATCHED).length, color: '#6366f1' },
