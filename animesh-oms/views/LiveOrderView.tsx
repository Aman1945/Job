...
const LiveOrderView: React.FC<LiveOrderViewProps> = ({ orders, onSelectOrder }) => {
  ...
  const getProgress = (status: OrderStatus) => {
    switch(status) {
      ...
      case OrderStatus.DISPATCHED: return 80;
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
                    <div className="flex items-center gap-4">
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-lg transition-colors ${
                        order.status === OrderStatus.OUT_FOR_DELIVERY || order.status === OrderStatus.PICKED_UP || order.status === OrderStatus.DISPATCHED ? 'bg-orange-600 text-white' : 
                        ...
                        'bg-slate-100 text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600'
                         ...
                         {order.status === OrderStatus.OUT_FOR_DELIVERY || order.status === OrderStatus.PICKED_UP || order.status === OrderStatus.DISPATCHED ? <Truck size={24} /> : order.status === OrderStatus.PENDING_PACKING ? <Box size={24} /> : <Clock size={24} />}
                      </div>
                      <div>
                         <div className="flex items-center gap-2">
                           ...
                           <span className={`text-[9px] font-black px-2 py-0.5 rounded-lg uppercase border ${
                             order.status === OrderStatus.OUT_FOR_DELIVERY || order.status === OrderStatus.PICKED_UP || order.status === OrderStatus.DISPATCHED ? 'bg-orange-50 text-orange-600 border-orange-100' : 
