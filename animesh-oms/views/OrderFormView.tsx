...
interface OrderFormViewProps {
  ...
  onBulkSubmit: (orders: Order[]) => void;
}

const OrderFormView: React.FC<OrderFormViewProps> = ({ customers, products, currentUser, allOrders, onSubmit, onBulkSubmit }) => {
  ...
  const getPreviousRate = (prodId: string, custId: string = selectedCustId) => {
    if (!custId) return 0;
  ...
  const handleBulkUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (evt) => {
      try {
        ...
        // Group data by CustomerID to create multiple orders
        ...
        const newOrders: Order[] = Object.entries(grouped).map(([custId, rows]: [string, any[]]) => {
          ...
          return {
            ...
            customerId: custId,
            customerName: customer?.name || `Unknown Client (${custId})`,
            items: rows.map(row => {
              const sku = String(row.SKUCode || row.sku_code || '').trim();
              const p = products.find(prod => prod.skuCode === sku || prod.id === sku);
              const qty = parseFloat(row.Quantity || 0);
              const rate = parseFloat(row.AppliedRate || row.rate || (p ? p.price : 0));
              const rowUnit = (String(row.Unit || row.unit || '').trim().toUpperCase() as any);
              const finalUnit = ['PCS', 'KG', 'PKT'].includes(rowUnit) ? rowUnit : (p?.unit || 'PCS');
              
              return {
                productId: p?.id || 'UNKNOWN',
                productName: p?.name || 'Unknown SKU',
                skuCode: sku,
                quantity: qty,
                unit: finalUnit,
                price: rate,
                baseRate: p?.baseRate || rate,
                previousRate: p ? getPreviousRate(p.id, custId) : 0,
                remarks: String(row.Remarks || ''),
                barcode: p?.barcode
              };
            }),
            status: OrderStatus.PENDING_CREDIT_APPROVAL,
            statusHistory: [{ status: OrderStatus.PENDING_CREDIT_APPROVAL, timestamp, userName: currentUser.name }],
            createdAt: timestamp,
            salespersonId: currentUser.id,
            generalRemarks: 'BULK IMPORT'
          };
        });

        onBulkSubmit(newOrders);
  ...
  const handleDownloadTemplate = () => {
    const ws = XLSX.utils.json_to_sheet([
      { CustomerID: '190068', SKUCode: '2004643', Quantity: 50, AppliedRate: 210, Unit: 'PCS', Remarks: 'Special rate approved' },
      { CustomerID: '190068', SKUCode: '2006332', Quantity: 10, AppliedRate: 540, Unit: 'KG', Remarks: '' },
      { CustomerID: '190190', SKUCode: '2004643', Quantity: 100, AppliedRate: 205, Unit: 'PKT', Remarks: 'Bulk priority' }
    ]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Bulk Template");
    XLSX.writeFile(wb, "NexusOMS_BulkOrder_Template.xlsx");
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (items.length === 0) {
      newErrors.push("Please add at least one line item.");
    }

    items.forEach((item, index) => {
      const prod = products.find(p => p.id === item.productId);
      const productName = prod ? prod.name : `Item ${index + 1}`;
      
      if (item.quantity <= 0) {
        newErrors.push(`${productName}: Quantity must be greater than 0.`);
      }
      
      const rate = parseFloat(item.appliedRate.toString());
      if (isNaN(rate) || rate <= 0) {
        newErrors.push(`${productName}: Applied rate must be a valid number greater than 0.`);
      }
    });
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-500 space-y-8">
      ...
      {/* Validation Errors */}
      {errors.length > 0 && (
        <div className="bg-rose-50 border-2 border-rose-200 rounded-[32px] p-8 animate-in slide-in-from-top-4 duration-500">
          <div className="flex items-center gap-3 mb-4 text-rose-600">
            <AlertCircle size={24} />
            <h4 className="text-lg font-black uppercase tracking-tight">Submission Blocked - Validation Errors</h4>
          </div>
          <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
            {errors.map((err, idx) => (
              <li key={idx} className="text-sm font-bold text-rose-700 flex items-center gap-2">
      ...
      <div className="bg-slate-900 rounded-[40px] p-8 text-white shadow-2xl relative overflow-hidden group">
         <Upload className="absolute -right-6 -bottom-6 w-32 h-32 opacity-10 group-hover:scale-110 transition-transform duration-700" />
         <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-8">
            <div>
               <h3 className="text-2xl font-black tracking-tight flex items-center gap-3">
                  <FileText className="text-emerald-400" /> Excel Bulk Import Hub
               </h3>
               <p className="text-sm text-slate-400 mt-1">Ingest multiple supply requests via standardized XLS template</p>
            </div>
            <div className="flex gap-4 w-full md:w-auto">
               <button 
                  onClick={handleDownloadTemplate}
                  className="flex-1 md:flex-none bg-white/10 hover:bg-white/20 border border-white/10 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2"
               >
                  <Download size={14}/> Download Template
               </button>
               <button 
                  onClick={() => bulkFileInputRef.current?.click()}
                  className="flex-1 md:flex-none bg-emerald-500 hover:bg-emerald-400 text-white px-8 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20"
               >
                  <Upload size={14}/> Upload Excel
               </button>
               <input type="file" ref={bulkFileInputRef} className="hidden" accept=".xlsx,.xls" onChange={handleBulkUpload} />
            </div>
         </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
          ...
          <div className="pt-10 border-t flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            >
              ...
              {isSubmitting ? 'Processing...' : 'Commit Supply Request'}
            </button>
          </div>
        </form>
      </div>
