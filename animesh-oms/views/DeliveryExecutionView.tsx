...
const DeliveryExecutionView: React.FC<DeliveryExecutionViewProps> = ({ orders, currentUser, onUpdateOrders, onOpenDetails }) => {
  ...
  const isAdmin = currentUser.role === UserRole.ADMIN || currentUser.role === UserRole.LOGISTICS;

  const myOrders = useMemo(() => {
    return (orders || []).filter(o => {
      // Admin and Logistics team should see the entire pipeline to monitor unassigned tasks
      if (isAdmin) return true;
      
      // Delivery Agents only see orders explicitly assigned to them
      const agentId = o.logistics?.deliveryAgentId;
      if (!agentId) return false;
      return agentId.trim().toLowerCase() === (currentUser.id || '').trim().toLowerCase();
    });
  }, [orders, currentUser, isAdmin]);

  const pickupOrders = useMemo(() => myOrders.filter(o => o.status === OrderStatus.READY_FOR_DISPATCH), [myOrders]);
  const inCustodyOrders = useMemo(() => myOrders.filter(o => o.status === OrderStatus.DISPATCHED || o.status === OrderStatus.PICKED_UP), [myOrders]);
  ...
  const updateOrderStatus = async (orderId: string, nextStatus: OrderStatus, extra: Partial<Order> = {}) => {
    ...
    try {
      ...
      const updatedOrders = orders.map(o => 
        o.id === orderId 
          ? { 
              ...
              agentConfirmedAt: nextStatus === OrderStatus.DISPATCHED ? timestamp : o.agentConfirmedAt,
              ...extra 
            } 
          : o
      );
      ...
      // Auto-switch tabs for better UX flow
      if (nextStatus === OrderStatus.DISPATCHED) setView('in-custody');
      if (nextStatus === OrderStatus.OUT_FOR_DELIVERY) setView('on-the-way');
      
    } catch (err) {
      console.error("Failed to update status:", err);
      alert("System error updating mission status. Please retry.");
    } finally {
    ...
    const updatedOrders = orders.map(o => {
      if (o.id === activeOrderId) {
        ...
        return { 
          ...
          rejectionReason: deliveryResult === OrderStatus.REJECTED ? rejectionReason : undefined, 
          items: updatedItems, 
          vaultTimestamp: timestamp, 
          cloudStoragePath: `NexusOMS/Vault/${activeOrder?.customerName}/${timestamp.split('T')[0]}/${activeOrderId}`, 
          statusHistory: [...(o.statusHistory || []), { status: deliveryResult, timestamp }] 
        };
      }
      return o;
    });
  ...
  const renderOrderCard = (order: Order, stage: string) => {
    const isUnassigned = !order.logistics?.deliveryAgentId;
    const orderValue = (order.items || []).reduce((s,i) => s + (i.price * (i.packedQuantity ?? i.quantity)), 0);
    
    return (
      <div key={order.id} className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden p-6 hover:shadow-md transition-all group">
        <div className="flex justify-between items-start mb-4">
          <div className="flex flex-col">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black text-emerald-600 font-mono tracking-widest">{order.id}</span>
              {isUnassigned ? (
                <span className="bg-amber-100 text-amber-700 text-[8px] font-black px-1.5 py-0.5 rounded uppercase border border-amber-200 flex items-center gap-1">
                  <AlertTriangle size={8} /> Unassigned
                </span>
              ) : (
                <span className="bg-indigo-50 text-indigo-600 text-[8px] font-black px-1.5 py-0.5 rounded uppercase border border-indigo-100 flex items-center gap-1">
                  <UserCheck size={8} /> Assigned
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-1.5 text-slate-400 text-[10px] font-black uppercase mt-1">
              <MapPin size={10} className="text-emerald-500" /> {order.warehouseSource || 'Dispatch Facility'}
            </div>
          </div>
          <div className="text-right">
            <p className="text-lg font-black text-slate-900">₹{orderValue.toLocaleString()}</p>
            <p className="text-[9px] font-bold text-slate-400 uppercase">{order.packedBoxes || 0} Box Units</p>
          </div>
        </div>
        ...
        {stage === 'pickup' && (
          <div className="space-y-3">
            {isUnassigned && isAdmin && (
              <div className="p-3 bg-amber-50 rounded-2xl border border-amber-100 flex items-center gap-3 text-amber-700">
                <AlertTriangle size={16} />
                <p className="text-[9px] font-bold uppercase leading-tight italic">Assignment missing. Admins can bypass Hub to start mission.</p>
              </div>
            )}
            <button 
              onClick={() => updateOrderStatus(order.id, OrderStatus.DISPATCHED)} 
              disabled={processingOrderId === order.id || (isUnassigned && !isAdmin)} 
              className={`w-full py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 disabled:opacity-30 shadow-lg shadow-emerald-600/10 ${
                isUnassigned ? 'bg-amber-600 text-white hover:bg-amber-500' : 'bg-emerald-600 text-white hover:bg-emerald-500'
              }`}
            >
              {processingOrderId === order.id ? <Clock className="animate-spin" size={16} /> : <CheckCircle2 size={16} />} 
              {isUnassigned ? "Bypass Assignment & Pickup" : "Confirm Pickup"}
            </button>
          </div>
        )}
        ...
        {stage === 'in-custody' && (
          <button 
            ...
            className="w-full bg-indigo-600 text-white py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-indigo-500 transition-all flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/10"
          >
            {processingOrderId === order.id ? <Clock className="animate-spin" size={16} /> : <Truck size={16} />} 
        ...
        {stage === 'on-the-way' && (
          <button 
            ...
            className="w-full bg-emerald-500 text-white py-4 rounded-2xl text-[11px] font-black uppercase tracking-widest hover:bg-emerald-400 shadow-lg transition-all flex items-center justify-center gap-2"
          >
            <CheckCircle2 size={16} /> Mark Fulfillment
          </button>
        )}
  ...
  return (
    <div className="max-w-md mx-auto space-y-6 pb-24 animate-in fade-in duration-500">
      <div className="flex bg-slate-200 p-1.5 rounded-3xl border border-slate-300 shadow-inner overflow-x-auto no-scrollbar">
        {[
          { id: 'pickup', label: 'Invoiced', count: pickupOrders.length },
          { id: 'in-custody', label: 'Dispatched', count: inCustodyOrders.length },
          ...
          <button 
            ...
            className={`flex-1 min-w-[80px] py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${view === tab.id ? 'bg-white shadow-md text-emerald-600' : 'text-slate-500'}`}
          >
            {tab.label} ({tab.count})
          </button>
        ))}
      ...
      <div className="space-y-4">
        ...
        {((view === 'pickup' && pickupOrders.length === 0) || (view === 'in-custody' && inCustodyOrders.length === 0) || (view === 'on-the-way' && activeDeliveries.length === 0) || (view === 'active-partials' && partiallyAccepted.length === 0)) && (
          <div className="py-24 text-center">
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Mission queue empty</p>
          </div>
        )}
      </div>
          ...
          <div className="bg-white w-full max-w-sm rounded-[44px] p-8 shadow-2xl relative overflow-hidden border border-white/20">
            {isVaulting ? (
              <div className="py-16 text-center space-y-8">
                <CloudUpload className="text-emerald-600 animate-bounce mx-auto" size={80} />
                <h4 className="text-2xl font-black text-slate-900 tracking-tighter">Archiving POD...</h4>
                <div className="w-full bg-slate-100 h-2 rounded-full overflow-hidden max-w-[240px] mx-auto shadow-inner">
                  <div className="h-full bg-emerald-600 animate-[progress_2s_ease-in-out]" style={{width: '100%'}} />
                </div>
              </div>
            ) : (
              <div className="space-y-8 max-h-[80vh] overflow-y-auto no-scrollbar">
                <div className="flex items-center justify-between">
                  <h4 className="text-2xl font-black text-slate-900 tracking-tight">Mission Closing</h4>
                  <button onClick={() => setActiveOrderId(null)} className="p-2 text-slate-400 hover:text-rose-500">
                    <XCircle size={24} />
                  </button>
                </div>
                <div className="space-y-3">
