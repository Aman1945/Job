...
const LogisticsAssignmentView: React.FC<LogisticsAssignmentViewProps> = ({ orders, users, onBulkUpdate }) => {
    ...
    const updatedOrders = orders.map(order => {
      if (selectedOrderIds.includes(order.id)) {
        ...
        return {
          ...order,
          logistics: {
            // Preservation logic: keep existing costs but update fleet info
            ...
            transporterId: order.logistics?.transporterId || 'Internal',
            ...order.logistics, // Spread existing for any custom properties
  ...
  const canSubmit = assignedAgentId && vehicleNo && selectedOrderIds.length > 0;
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-24 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex bg-slate-200 p-1 rounded-2xl border border-slate-300 shadow-inner overflow-x-auto no-scrollbar">
           <button onClick={() => setActiveTab('pending')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'pending' ? 'bg-white shadow-md text-emerald-600' : 'text-slate-500'}`}>Unassigned ({pendingOrders.length})</button>
           <button onClick={() => setActiveTab('active')} className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${activeTab === 'active' ? 'bg-white shadow-md text-emerald-600' : 'text-slate-500'}`}>Track Trips ({activeShipments.length})</button>
        </div>
      </div>
          ...
          {activeTab === 'pending' ? (
            ...
            <div className="space-y-4 animate-in slide-in-from-right-4">
              {activeShipments.map(order => {
                ...
                const orderVal = (order.items || []).reduce((s,i)=>s+(i.price*(i.packedQuantity || i.quantity)),0);
                return (
                  <div key={order.id} className="bg-white rounded-[32px] border border-slate-200 overflow-hidden shadow-sm group hover:border-emerald-200 transition-all">
                     <div className="p-6 flex flex-col md:flex-row items-center gap-6">
                        ...
                        <div className="text-right shrink-0">
                           <p className="text-xl font-black text-slate-900">₹{orderVal.toLocaleString()}</p>
          ...
          <div className="bg-emerald-950 rounded-[40px] p-8 text-white shadow-2xl border border-emerald-900 relative overflow-hidden group">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <select className={`w-full bg-white/5 border rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-emerald-500/50 outline-none appearance-none transition-all ${assignedAgentId ? 'border-white/20 text-white' : 'border-rose-500/30 text-rose-300'}`} value={assignedAgentId} onChange={(e) => setAssignedAgentId(e.target.value)}>
                    <option value="" className="bg-emerald-950 text-slate-500">Select Agent...</option>
              ...
              <div className="space-y-2">
                ...
                <input type="text" placeholder="e.g. MH-12-Nexus-1234" className={`w-full bg-white/5 border rounded-2xl px-6 py-4 text-sm font-bold focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all uppercase placeholder:text-emerald-900/40 ${vehicleNo ? 'border-white/20 text-white' : 'border-rose-500/30 text-rose-300'}`} value={vehicleNo} onChange={(e) => setVehicleNo(e.target.value)} />
              </div>
              <div className="space-y-2">
              ...
              {selectedOrderIds.length > 0 && !canSubmit && (
